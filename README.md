...
В меню **Настройки → FTP** сохраняются параметры FTP/FTPS, проверяется подключение и доступна история запусков. Backend ежедневно в `23:55` по часовому поясу `AUTOLOAD_TIMEZONE` проверяет указанный каталог и обрабатывает все новые `.xls`/`.xlsx`/`.html`/`.htm`. После полностью успешного импорта удалённый файл удаляется. При ошибке файл остаётся на FTP и переименовывается с префиксом `EROOR_`, чтобы не загружаться повторно автоматически. Документы с номерами, начинающимися на `ВЗВ-`, `РНВ-` или `ВРМ-`, учитываются как пропущенные и в журнал продаж не добавляются.
