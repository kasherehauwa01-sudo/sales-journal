...
async def get_buyer_type_clients(buyer_type:str):
 encoded=quote(buyer_type,safe="")
 params=urlencode({"buyer_type":buyer_type})

 # Используем облегчённый endpoint Clients VR, который возвращает
 # только уникальные имена клиентов без полных карточек.
 paths=[
  f"/integration/buyer-types/{encoded}/client-names",
  f"/integration/buyer-types/{encoded}/clients",
  f"/integration/clients?{params}",
  f"/buyer-types/{encoded}/clients",
  f"/clients?{params}",
 ]

 return await _cached(
  f"buyer-type:{buyer_type.strip().casefold()}",
  lambda:_items(
   _request(paths),
   ("client","name","client_name","full_name","Клиент"),
  ),
 )
