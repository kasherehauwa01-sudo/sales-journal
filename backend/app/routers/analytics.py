...
@router.get("/overview")
async def overview(date_from:date|None=None,date_to:date|None=None,department:str|None=None,client:str|None=None,price_type:str|None=None,promotion:str|None=None,db:AsyncSession=Depends(get_db)):
 current=await metrics(db,filters(select(Sale),date_from,date_to,department,client,price_type,promotion));previous={}
...
@router.get("/products")
async def products(date_from:date|None=None,date_to:date|None=None,department:str|None=None,limit:int=Query(100,le=500),db:AsyncSession=Depends(get_db)):
 q=filters(select(SaleItem.article,SaleItem.code,SaleItem.name,func.sum(SaleItem.quantity*SaleItem.actual_price).label("revenue"),func.sum(SaleItem.quantity).label("units"),func.count(distinct(SaleItem.sale_id)).label("checks"),func.avg(SaleItem.actual_price).label("average_price"),func.avg(SaleItem.base_price).label("average_base_price"),func.count(distinct(Sale.department)).label("departments")).join(Sale,Sale.id==SaleItem.sale_id).group_by(SaleItem.article,SaleItem.code,SaleItem.name).order_by(func.sum(SaleItem.quantity*SaleItem.actual_price).desc()).limit(limit),date_from,date_to,department)
